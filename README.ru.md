...
Для `snake` параметр `birthFrames` задает появление тела из букв ника, а `fieldRevealFrames` управляет тем, насколько медленно появляется остальное поле с едой. `length` задает длину основной змейки, `maxLength` ограничивает рост, а `growPerFood` задает, насколько змейка растет после съеденной клетки. Значение `0` оставляет ее короткой: она ест клетки, но почти не растет. Быстрый червяк настраивается через `worm`, `wormLength`, `wormSpeed` и `wormGrowPerFood`.
