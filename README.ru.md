...
- `snake`: змейка с розовой головой стартует после интро с ником и сначала ест самые темные клетки.
...
Для `snake` параметр `length` задает стартовую длину, `maxLength` ограничивает рост, а `growPerFood` задает, насколько змейка растет после съеденной клетки.
