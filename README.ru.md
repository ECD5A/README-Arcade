...
- `snake`: короткая змейка с розовой головой стартует после интро с ником, сначала ест самые темные клетки и не забивает собой всю сетку.
...
Для `snake` параметр `length` задает стартовую длину, `maxLength` ограничивает рост, а `growPerFood` задает, насколько змейка растет после съеденной клетки. Значение `0` оставляет ее короткой: она ест клетки, но почти не растет.
