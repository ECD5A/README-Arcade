...
- `snake`: короткая змейка с розовой головой и быстрый червяк появляются из клеток ника, сначала едят самые темные клетки и не забивают собой всю сетку.
- `matrix`: вертикальный code rain, который накрывает интро с ником сверху вниз и оставляет GitHub-style хвосты.
- `defrag`: карта диска в духе Windows 98, где фрагменты постепенно уплотняются в GitHub-цветах.
...
`transitionFrames` задает количество кадров для мягкого перехода от ника к режимам, которые поддерживают такой старт.

Для `snake` параметр `birthFrames` задает появление тела из букв ника, `fieldRevealFrames` управляет скоростью появления поля с едой, а `edgeRunDelay` / `edgeRunFrames` позволяют двум настоящим змейкам ненадолго приоритизировать верхнюю и нижнюю строку во время игры. `length` задает длину основной змейки, `maxLength` ограничивает рост, а `growPerFood` задает, насколько змейка растет после съеденной клетки. Значение `0` оставляет ее короткой: она ест клетки, но почти не растет. Быстрый червяк настраивается через `worm`, `wormLength`, `wormSpeed` и `wormGrowPerFood`.
