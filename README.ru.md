...
Для `snake` параметр `birthFrames` задает появление тела из букв ника, `fieldRevealFrames` управляет скоростью появления поля с едой, а `edgeSweepFrames` добавляет короткий проход по верхней и нижней линии. `length` задает длину основной змейки, `maxLength` ограничивает рост, а `growPerFood` задает, насколько змейка растет после съеденной клетки. Значение `0` оставляет ее короткой: она ест клетки, но почти не растет. Быстрый червяк настраивается через `worm`, `wormLength`, `wormSpeed` и `wormGrowPerFood`.
